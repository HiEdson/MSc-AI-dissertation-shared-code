...
def evaluate(loader, head, dev, K, C, Q):
    """Val loss is teacher-forced; accuracy is from GREEDY generation (head(x) with no
    teacher tokens) so independent- and dep-heads are compared on deployable numbers."""
    ...
    with torch.no_grad():
        for xb, yb, y0b in loader:
            ...
            B = xb.shape[0]
            y_tf = yb.permute(0, 3, 1, 2)                 # [B,K,C,Q] teacher tokens
            tot_loss += tpp_loss(head(xb, y_tf), yb).item() * B
            n += B
            pred = head(xb).argmax(-1)                     # greedy/one-shot -> [B,K,C,Q]
            tgt = y_tf                                     # [B,K,C,Q]
...
def main() -> None:
    ...
    ap.add_argument("--head", choices=["independent", "dep"], default="independent",
                    help="independent = v0 (codebooks independent); dep = v1 (autoregressive over codebooks)")
    ...
    from duplex_spec.head import MultiStepTPPHead, MultiStepDepHead, tpp_loss
    ...
    dev = args.device
    Head = MultiStepDepHead if args.head == "dep" else MultiStepTPPHead
    head = Head(hidden_dim=H, n_channels=C, n_codebooks=Q, codebook_size=2048, horizon=K).to(dev)
    ...
    for ep in range(args.epochs):
        ...
        if vl < best_val:
            ...
            flag = "   *best*"
        print(f"[ep {ep}] train={tot/ntr:.3f} val={vl:.3f} | head(k1..k{K})={fmt(hacc)} | copy={fmt(cacc)}{flag}")
