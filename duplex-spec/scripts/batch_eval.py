...
def main():
    ...
    for st in starts:
        ...
        for t in trigs:
            ...
            print(f"  {t:>8}: lat={met['response_latency_ms']:.0f}ms "
                  f"overlap={met['overlap_pct']:.1f}% talk={met['moshi_talk_pct']:.1f}%")
    ...
    keys = ["start", "trigger", "response_latency_ms", "overlap_pct", "dead_air_pct",
            "moshi_talk_pct", "n_moshi_turns", "n_responses"]
    ...
    print(f"{'trigger':>10} {'lat_ms':>16} {'overlap%':>14} {'talk%':>14} {'clips':>6}")
    print("-"*66)
    agg = {}
    base_lat = None
    for t in trigs:
        ...
        lat = np.array([r["response_latency_ms"] for r in rows if r["response_latency_ms"]==r["response_latency_ms"]])
        ov = np.array([r["overlap_pct"] for r in rows])
        tk = np.array([r["moshi_talk_pct"] for r in rows])
        agg[t] = dict(lat_mean=float(lat.mean()) if len(lat) else float('nan'),
                      lat_std=float(lat.std()) if len(lat) else float('nan'),
                      overlap_mean=float(ov.mean()), overlap_std=float(ov.std()),
                      talk_mean=float(tk.mean()), talk_std=float(tk.std()), n=len(rows))
        if t == "baseline": base_lat = agg[t]["lat_mean"]
    for t in trigs:
        if t not in agg: continue
        a = agg[t]
        delta = ""
        if base_lat is not None and t != "baseline" and a["lat_mean"]==a["lat_mean"]:
            d = a["lat_mean"] - base_lat
            delta = f" ({'+' if d>=0 else ''}{d:.0f})"
        print(f"{t:>10} {a['lat_mean']:>7.0f}±{a['lat_std']:<5.0f}{delta:>8} "
              f"{a['overlap_mean']:>7.1f}±{a['overlap_std']:<4.1f} "
              f"{a['talk_mean']:>7.1f}±{a['talk_std']:<4.1f} {a['n']:>6}")
