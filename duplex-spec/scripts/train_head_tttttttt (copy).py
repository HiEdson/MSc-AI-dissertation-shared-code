...
python scripts/train_head.py --pairs-dir pairs/ --head dep --epochs 15 --device cuda \
  --out head_v1.pt --save-json train_v1.json
import argparse
import json
import sys
...
def main() -> None:
    ...
    from duplex_spec.head_exp2 import MultiStepTPPHead, MultiStepDepHead, tpp_loss
    ...
    head = Head(
        ...
        codebook_size=2048, 
        horizon=K,
        trunk_dim=1024,   # Narrowed down from 2048
        dep_dim=512,      # Narrowed down from 1024
        dep_layers=8,     # Doubled depth from 4
        dep_heads=8
    ).to(dev)
