...
def main():
    ...
    def run_conv(tokens, train: bool, n_prefill: int = 0):
        """One conversation window: live forward + loss. Returns (sum_loss, n_examples).
        The first `n_prefill` frames only warm the KV cache (no loss, no grad)."""
        T = tokens.shape[2]
        feats, tgts = [], []
        ...
        with lm_gen.streaming(1):
            for t in range(T):
                ...
                if t < n_prefill:                       # warm the cache only
                    with torch.no_grad():
                ...
                if out is None:
                    continue
                _tok, hstate = out
                h = hstate[:, 0] if hstate.dim() == 3 else hstate      # [1, H]
                if t + K < T:
                    feats.append(h)
                    tgts.append(tokens[:, :, t + 1:t + 1 + K])         # [2, Q, K]
                if len(feats) >= args.batch or t == T - 1:
                    if not feats:
                        continue
                    hb = torch.cat(feats, 0).float()                   # match fp32 head
                    ...
                    if torch.isfinite(loss):
                        if train:
                            ...
                            detach_streaming(lm)
                            if not checked["done"]:     # one-time gradient sanity check
                                g = adapters[0].B.grad
                                print(f"[chk] hb.requires_grad={hb.requires_grad} "
                                      f"| A.grad={'None' if g is None else f'{g.norm().item():.3e}'}")
                                if g is None or not hb.requires_grad:
                                    print("[chk] WARNING: no gradient reaching the LoRA "
                                          "adapters --- they are NOT training. Fix before "
                                          "trusting any result.")
                                checked["done"] = True
                        tot += loss.item() * len(feats); n += len(feats)
                    feats, tgts = [], []
                    local_step += 1
