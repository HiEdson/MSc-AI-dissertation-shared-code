...
def cmd_fit(args):
    ...
    # keep per-conversation arrays (NOT overlapping windows) to bound memory: a window is
    # sliced lazily at access time. Overlapping h_t windows would be ~10GB/conversation.
    feats_list, hmat_list, tgt_list = [], [], []
    index = []                                              # (conv_i, end_frame) per window
    for cid, npz, npy, lab in iter_convs(args.pairs_dir, args.labels_dir):
        ...
        if len(feat) < args.win: continue
        ci = len(feats_list)
        feats_list.append(feat.astype(np.float32))
        hmat_list.append(hmat.astype(np.float32))
        tgt_list.append(tgt.astype(np.float32))
        index += [(ci, e) for e in range(args.win - 1, len(feat))]
        print(f"[data] {cid}: {len(feat) - args.win + 1} windows")
    n_win = len(index)
    if n_win == 0:
        sys.exit("no windows")
    n_feat = feats_list[0].shape[-1]
    ymean = float(np.mean([t.mean() for t in tgt_list]))
    print(f"[fit] {n_win} windows (lazy), target mean~={ymean:.1f} (cap={args.cap})")

    # standardise features from a SAMPLE (avoid concatenating everything)
    sample = np.concatenate([f for f in feats_list])       # [sum_T, n_feat] -- feat only, small
    mu = sample.mean(0); sd = sample.std(0) + 1e-6
    del sample

    class LazyWin(torch.utils.data.Dataset):
        def __init__(self, win): self.win = win
        def __len__(self): return n_win
        def __getitem__(self, k):
            ci, e = index[k]; w = self.win
            f = (feats_list[ci][e - w + 1:e + 1] - mu) / sd   # [win, n_feat]
            h = hmat_list[ci][e - w + 1:e + 1]                # [win, 4096]
            y = tgt_list[ci][e]
    ...
    dl = DataLoader(LazyWin(args.win), batch_size=args.batch, shuffle=True, num_workers=2)
...
def cmd_eval(args):
    ...
    for cid, npz, npy, lab in iter_convs(args.pairs_dir, args.labels_dir):
        ...
        if len(feat) < win: continue
        ends = list(range(win - 1, len(feat)))
        # slice + predict in batches, one conversation at a time (bounded memory)
        for s0 in range(0, len(ends), args.batch):
            be = ends[s0:s0 + args.batch]
            Fb = np.stack([(feat[e - win + 1:e + 1] - mu) / sd for e in be]).astype(np.float32)
            Hb = np.stack([hmat[e - win + 1:e + 1] for e in be]).astype(np.float32)
            with torch.no_grad():
                p = model(torch.from_numpy(Fb).to(args.device),
                          torch.from_numpy(Hb).to(args.device)).cpu().numpy()
