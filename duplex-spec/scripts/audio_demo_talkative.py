...
def main():
    ...
    ap.add_argument("--start", default=None,
                    help="explicit clip start frame(s), comma-separated; overrides label picking")
    ap.add_argument("--user-channel", type=int, default=1,
                    help="which token channel is fed to Moshi as the USER (default 1 = the "
                         "more talkative speaker; Moshi generates the OTHER channel)")
    ...
    U = args.user_channel                 # channel fed to Moshi as the user
    M = 1 - U                             # channel Moshi generates (the other speaker)
    ...
    def user_frame(t):
        return torch.from_numpy(tokens[U, :, t]).to(dev).long()[None, :, None][:, :needed]
    def silence_moshi():
        # each of Moshi's dep codebooks gets its OWN silence token, not cb0's for all
        vals = torch.tensor(sil_cb[M, :lm.dep_q], dtype=torch.long, device=dev)
    ...
    def release_schedule(trigger, start, stop, handoffs):
        ...
        prev = None; armed = True; silent_run = 0
        for t in range(start, stop):
            user_active = tokens[U, 0, t] != sil[U]
            fire, prev = gate_condition(trigger, t, prev)
            i = t - start
            if armed and user_active and fire:
                speaking[i] = True
                armed = False; silent_run = 0
            elif not armed:
    ...
    def generate(speaking, start, stop):
        ...
        user_codes = tokens[U, :Q, start:stop].astype(np.int64)  # [Q, win] real user audio (ch U)
    ...
    if args.start is not None:
        ...
        pick = np.array([s + args.clip_frames // 2 for s in starts])  # centre so window = [start, start+frames]
    else:
    ...
    for n, h in enumerate(pick):
        ...
        clip_ho = [int(x) for x in ho if start <= int(x) < stop] or [int(h)]
        entry = {"clip": n, "handoff_frames": clip_ho, "turns": {}}
        for trig in trigs:
